...
func (player *Player) loseHealth(amount uint) {
	if player.health-amount > player.health {
		player.health = 0
	} else {
		player.health -= amount
	}
	fmt.Println(player.name, "Lose", amount, " -> ", player.health)
}
...
func (player *Player) LoseEnergy(amount uint) {
	if player.energy-amount > player.energy {
		player.energy = 0
	} else {
		player.energy -= amount
	}
	fmt.Println(player.name, "Lose", amount, "energy -> ", player.energy)
}
